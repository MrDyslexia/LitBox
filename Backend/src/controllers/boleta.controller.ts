...
export async function crearBoleta(
  ...
  // lean() para devolver objeto plano serializable como JSON
  return Boleta.findById(boleta._id).lean()
}
...
export async function cambiarEstado(
  id: string,
  accion: "revisar" | "aprobar" | "rechazar",
  ...
  // lean() para objeto plano + populate manual
  return Boleta.findById(boleta._id)
    .populate("empleado", "nombre email avatar")
    .populate("auditor", "nombre email avatar")
    .lean()
}
...
export async function estadisticas(authUser: AuthUser) {
  ...
  return {
    ...
    montoAprobado: montos[0]?.totalMonto ?? 0,
  }
}
